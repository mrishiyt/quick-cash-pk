...
// Main Admin / Master Referral Code (Is se pehla banda register hoga)
...
async function registerUser() {
    try {
        ...
        // Check if entered code is Master Code OR an existing user's code
        if (refBy === MASTER_REF_CODE) {
            ...
            if (!refQuery.empty) {
                isRefValid = true;
                // Add 100 App PKR bonus to Referrer
...
function updateUI() {
    const realCash = (currentUser.balance * 0.05).toFixed(1); // 1000 App PKR = 50 PKR Real Cash
...
async function watchAd() {
    ...
    setTimeout(async () => {
        ...
        currentUser.balance += 10; // 10 App PKR per ad
